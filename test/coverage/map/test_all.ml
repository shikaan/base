...
let%expect_test "sexp_of_t" =
  ...
  print_s [%sexp (map : (string, int, (_[@sexp.phantom])) Map.t)];
  [%expect
    {|
    ((a 1)
     (b 2)
...
module Tree = struct
  ...
  let%expect_test _ =
    quickcheck_m (module Tree_int) ~f:(fun tree ->
      let tree = Tree_int.value tree in
      let sexp = sexp_of_t Int.sexp_of_t Int.sexp_of_t tree in
      require_equal
        (module Sexp)
        sexp
...
module Using_comparator = struct
  ...
  let%expect_test _ =
    quickcheck_m (module Instance_int) ~f:(fun t ->
      let t = Instance_int.value t in
      let sexp = sexp_of_t Int.sexp_of_t Int.sexp_of_t t in
