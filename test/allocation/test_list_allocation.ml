...
module%test From_local_iterators_to_be_replaced = struct
  module Test_result : sig
    ...
    val with_output : ((Sexp.t -> unit) -> 'a) -> 'a t
  ...
  module%test [@name "append_local"] _ = struct
    let append_local x y =
      Expect_test_helpers_core.require_no_allocation_local (fun () -> append_local x y)
    ;;
  ...
  module%test [@name "concat_local"] _ = struct
    let concat_local x =
      Expect_test_helpers_core.require_no_allocation_local (fun () -> concat_local x)
    ;;
      ...
      ignore
        (Expect_test_helpers_core.require_no_allocation_local (fun () ->
           filter_mapi_local example ~f:(fun index i ->
             if i > 50 then Some (index + 1) else None))
         : int list);
      let expected =
        Test_result.with_output (fun output ->
  ...
  let%expect_test "filter_map_local" =
    ...
    List.iter examples ~f:(fun example ->
      ...
      ignore
        (Expect_test_helpers_core.require_no_allocation_local (fun () ->
           filter_map_local example ~f:(fun i -> if i > 10 then Some (i + 1) else None))
         : int list);
      let expected =
        Test_result.with_output (fun output ->
      ...
      ignore
        (Expect_test_helpers_core.require_no_allocation_local (fun () ->
           rev_filter_map_local example ~f:(fun i ->
             if i > 10 then Some (i + 1) else None))
         : int list);
      let expected =
        Test_result.with_output (fun output ->
      ...
      ignore
        (Expect_test_helpers_core.require_no_allocation_local (fun () ->
           filter_map_local_output example ~f:(fun i ->
             if i > 50 then Some (i + 1) else None))
         : int list);
      let expected =
        Test_result.with_output (fun output ->
          List.filter_map example ~f:(f output :> _ -> _) [@nontail])
  ...
  module%test [@name "filter_opt_local"] _ = struct
    let filter_opt_local x =
      Expect_test_helpers_core.require_no_allocation_local (fun () -> filter_opt_local x)
    ;;
  ...
  let%expect_test "concat_map_local" =
    ...
    List.iter examples ~f:(fun example ->
      ...
      ignore
        (Expect_test_helpers_core.require_no_allocation_local (fun () ->
           concat_map_local example ~f:(fun i -> [ i; i * i ]))
         : T.t);
      let expected =
        Test_result.with_output (fun output ->
  ...
  let%expect_test "partition_map_local" =
    ...
    List.iter examples ~f:(fun example ->
      ...
      ignore
        (Expect_test_helpers_core.require_no_allocation_local (fun () ->
           partition_map_local example ~f:(fun i ->
             if i > 50 then First (i * 100) else Second (-i)))
         : T.t);
      let expected =
        Test_result.with_output (fun output ->
  ...
  module%test [@name "cartesian_product_local"] _ = struct
    let cartesian_product_local x y =
      Expect_test_helpers_core.require_no_allocation_local (fun () ->
  ...
  let%expect_test "find_map_local" =
    ...
    List.iter examples ~f:(fun example ->
      ...
      ignore
        (Expect_test_helpers_core.require_no_allocation_local (fun () ->
           find_map_local example ~f:(fun i -> if i > 50 then Some (i + 10) else None))
         : int option);
      let expected =
        Test_result.with_output (fun output ->
  ...
  let%expect_test "fold_map_local" =
    ...
    List.iter examples ~f:(fun example ->
      ...
      ignore
        (Expect_test_helpers_core.require_no_allocation_local (fun () ->
           fold_map_local example ~init ~f:(fun acc i -> i - acc, acc))
         : T.t);
      let expected =
        Test_result.with_output (fun output ->
  ...
  let%expect_test "fold_until_local" =
    ...
    List.iter examples ~f:(fun example ->
      ...
      ignore
        (Expect_test_helpers_core.require_no_allocation_local (fun () ->
           fold_until_local example ~init ~finish:Fn.id ~f:(fun acc i ->
             let acc = acc + i + 1 in
             if acc >= 100 then Continue_or_stop.Stop acc else Continue acc))
         : T.t);
      let expected =
  ...
  let%expect_test "init_local_i64" =
    ...
    List.iter [ 0L; 1L; 10L ] ~f:(fun example_len ->
      ...
      ignore
        (Expect_test_helpers_core.require_no_allocation_local (fun () ->
           init_local_i64 (I64.unbox example_len) ~f:(fun x -> I64.box x))
         : T.t);
      let expected =
        Test_result.with_output (fun output ->
          List.init
  ...
  module%test [@name "nth_local"] _ = struct
    let nth_local x y =
      Expect_test_helpers_core.require_no_allocation_local (fun () -> nth_local x y)
    ;;
  ...
  let%expect_test "nth_local_exn" =
    ...
    List.iter examples ~f:(fun example ->
      ...
      then
        ...
        ignore
          (Expect_test_helpers_core.require_no_allocation_local (fun () ->
             map_local [ 0; 1; 2 ] ~f:(fun n -> nth_local_exn example n))
           : T.t);
      let expected =
        Test_result.with_output (fun output ->
  ...
  let chunks_of t ~length = (List.chunks_of [@alloc stack]) t ~length
  ...
  module%test [@name "rev_local"] _ = struct
    let rev_local x =
      Expect_test_helpers_core.require_no_allocation_local (fun () -> rev_local x)
    ;;
  ...
  module%test [@name "take_local"] _ = struct
    let take_local x y =
      Expect_test_helpers_core.require_no_allocation_local (fun () -> take_local x y)
    ;;
  ...
  module%test [@name "transpose_local"] _ = struct
    let transpose_local x =
      Expect_test_helpers_core.require_no_allocation_local (fun () -> transpose_local x)
    ;;
  ...
  module%test [@name "unzip_local"] _ = struct
    let unzip_local x =
      Expect_test_helpers_core.require_no_allocation_local (fun () -> unzip_local x)
    ;;
    ...
    let%expect_test "min_elt_local" =
      ...
      List.iter compares ~f:(fun compare ->
        ...
        List.iter examples ~f:(fun example ->
          let result =
            [%globalize: T.t]
              (min_elt_local example ~compare:(compare :> int -> int -> int))
          in
    ...
    let%expect_test "max_elt_local" =
      ...
      List.iter compares ~f:(fun compare ->
        ...
        List.iter examples ~f:(fun example ->
          let result =
            [%globalize: T.t]
              (max_elt_local example ~compare:(compare :> int -> int -> int))
          in
